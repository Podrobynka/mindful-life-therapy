...
class AboutPageControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should update about_page" do
    patch about_page_url, params: { about_page: { body: about_pages(:one).body, page_photo: fixture_file_upload(photo_fixture, 'image/png') } }
    assert_redirected_to about_page_url
  end

  test "should update about_page via :xhr" do
    patch about_page_url, xhr: true, params: { about_page: { body: about_pages(:one).body, page_photo: fixture_file_upload(photo_fixture, 'image/png') } }
  ...
  test "should show error message when update fails" do
    patch about_page_url, params: { about_page: { body: '', page_photo: fixture_file_upload(photo_fixture, 'image/png') } }
  ...
  test "should show error message when update via :xhr fails" do
    ...
    patch about_page_url, xhr: true, params: { about_page: { body: '', page_photo: fixture_file_upload(photo_fixture, 'image/png') } }
    assert_match /1 error prohibited this record from being saved/, response.body
  end
end
